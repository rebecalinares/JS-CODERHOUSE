...
const logIn = () => {
    let name = prompt("Ingrese su nombre (en minísculas y sin tildes) o escriba 0 para salir: ").toLowerCase();
    if(name === "") {
        do {
            alert("No ha ingresado ningún nombre. Intente nuevamente...");
            ...
            if(name === admin.name) {
                ...
                for (i = 0; i < 3; i++) {
                    if (validacion (password, admin.password) === true) {
                        i = 3;
                        alert(`¡Hola ${name}, bienvenido de vuelta!`);
                        menu2 ();
                        opcion = "0";
                    }else {
...
alert("¡Bienvenido! A continuación identifíquese para realizar su compra.");
